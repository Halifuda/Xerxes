/// TODO: deprecated, find another implementation.
#pragma once
  ...
  public:
    ...
    void transit() override {
        ...
        if (upstreams.find(pkt.from) == upstreams.end()) {
            XerxesLogger::debug()
                << name() << " directly send packet " << pkt.id
        ...
        if (packages[cur_pkg_id].sub_pkts.size() == packaging_num) {
            for (auto &sub_pkt : packages[cur_pkg_id].sub_pkts) {
                sub_pkt.second.delta_stat(
                    PACKAGING_DELAY, (double)(tick > sub_pkt.second.arrive
                                                  ? tick - sub_pkt.second.arrive
                                                  : 0));
                sub_pkt.second.arrive =
                    (tick > sub_pkt.second.arrive ? tick
                                                  : sub_pkt.second.arrive);
                XerxesLogger::debug() << name() << " send packet "
                                      << sub_pkt.second.id << std::endl;
            ...
            if (!pkt.is_rsp) {
                ...
                for (size_t i = 0; i < pkt.burst; i++) {
                    auto new_pkt = PktBuilder()
                                       ...
                                       .burst(pkt.burst) // Remain the same for
                                                         // filtering the rsp
                                       .type(pkt.type)
                                       .build();
                    bursts[id].sub_pkts.insert(new_pkt.id);
                    reverse[new_pkt.id] = id;
                    XerxesLogger::debug() << name() << " send sub-packet "
                                          << new_pkt.id << std::endl;
                    send_pkt(new_pkt);
                }
            } else {
                ...
                ASSERT(it != reverse.end(),
                       name() + " cannot find recorded sub-pkt " +
                           std::to_string(id));
                ...
                rec.origin.delta_stat(DRAM_TIME,
                                      (double)pkt.get_stat(DRAM_TIME));

                rec.sub_pkts.erase(id);
                if (rec.sub_pkts.empty()) {
                    XerxesLogger::debug() << name() << " send origin packet "
                                          << rec.origin.id << std::endl;
