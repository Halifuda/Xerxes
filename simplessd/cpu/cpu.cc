...
CPU::Core::Core() : busy(false), ignoreScheduling(false), lastBusy(0) {
...
CPU::CPU(ConfigReader &c) : conf(c), lastResetStat(0) {
  ...
  eccClockSpeed = conf.readUint(CONFIG_CPU, CPU_ECC_CLOCK);
  eccClockPeriod = 1000000000000. / eccClockSpeed;
  ...
  eccCore.resize(conf.readUint(CONFIG_CPU, CPU_CORE_ECC));
  ...
  cpi.insert({ECC, std::unordered_map<uint16_t, InstStat>()});
  ...
  cpi.find(13)->second.insert({41, InstStat(5, 16, 3, 16, 0, 0, eccClockPeriod)});
  cpi.find(13)->second.insert({42, InstStat(18, 60, 10, 122, 126, 2, eccClockPeriod)});
}
...
void CPU::calculatePower(Power &power) {
  ...
  uint32_t totalCore = hilCore.size() + iclCore.size() + ftlCore.size() + eccCore.size();
  uint32_t coreIdx = 0;
...
void CPU::applyLatency(NAMESPACE ns, FUNCTION fct, uint64_t &tick) {
  Core *pCore = nullptr;

  // std::printf("applyLatency %d %d\n", ns, fct);
  ...
  switch (ns) {
    ...
    case ECC:
      if (eccCore.size() > 0) {
        pCore = &eccCore.at(leastBusyCPU(eccCore));
      }
  ...
  if (pCore) {
    ...
    if(pCore->ignoreScheduling) {
      tick += inst->second.latency;
    }
    else {
      if (pCore->lastBusy <= tick) {
        pCore->lastBusy = tick + inst->second.latency;
      }
      else {
        pCore->lastBusy += inst->second.latency;
      }
...
void CPU::getStatList(std::vector<Stats> &list, std::string prefix) {
  ...
  for (uint32_t i = 0; i < eccCore.size(); i++) {
    number = std::to_string(i);

    temp.name = prefix + ".ecc" + number + ".busy";
    temp.desc = "CPU for ECC core " + number + " busy ticks";
    list.push_back(temp);

    temp.name = prefix + ".ecc" + number + ".insts.branch";
    temp.desc = "CPU for ECC core " + number + " executed branch instructions";
    list.push_back(temp);

    temp.name = prefix + ".ecc" + number + ".insts.load";
    temp.desc = "CPU for ECC core " + number + " executed store instructions";
    list.push_back(temp);

    temp.name = prefix + ".ecc" + number + ".insts.store";
    temp.desc = "CPU for ECC core " + number + " executed load instructions";
    list.push_back(temp);

    temp.name = prefix + ".ecc" + number + ".insts.arithmetic";
    temp.desc =
        "CPU for ECC core " + number + " executed arithmetic instructions";
    list.push_back(temp);

    temp.name = prefix + ".ecc" + number + ".insts.fp";
    temp.desc =
        "CPU for ECC core " + number + " executed floating point instructions";
    list.push_back(temp);

    temp.name = prefix + ".ecc" + number + ".insts.others";
    temp.desc = "CPU for ECC core " + number + " executed other instructions";
    list.push_back(temp);
  }
}
...
void CPU::printLastStat() {
  debugprint(LOG_CPU, "Ignore CPU power calculation");
  return;
  Power power;
