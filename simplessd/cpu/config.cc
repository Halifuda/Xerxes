...
bool Config::setConfig(const char *name, const char *value) {
  ...
  if (MATCH_NAME(NAME_ECC_CLOCK)) {
    eccClock = strtoul(value, nullptr, 10);
  }
  ...
  else if (MATCH_NAME(NAME_CORE_ECC)) {
    eccCore = (uint32_t)strtoul(value, nullptr, 10);
  }
  else {
    ret = false;
  }
