...
const char NAME_WRITE_POLICY[] = "WritePolicy";
...
Config::Config() {
  ...
  writeCaching = true;
  readPrefetch = false;
  writePolicy = POLICY_WRITE_THROUGH;
...
bool Config::setConfig(const char *name, const char *value) {
  ...
  else if (MATCH_NAME(NAME_WRITE_POLICY)) {
    writePolicy = (WRITE_POLICY)strtoul(value, nullptr, 10);
  }
