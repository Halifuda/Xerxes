...
const char SECTION_COUNT[] = "count";
...
const char SECTION_ERROR[] = "error";
...
int64_t ConfigReader::readInt(CONFIG_SECTION section, uint32_t idx) {
  switch (section) {
    case CONFIG_COUNT:
      return countConfig.readInt(idx);
    case CONFIG_CPU:
...
uint64_t ConfigReader::readUint(CONFIG_SECTION section, uint32_t idx) {
  switch (section) {
    case CONFIG_COUNT:
      return countConfig.readUint(idx);
    case CONFIG_CPU:
...
float ConfigReader::readFloat(CONFIG_SECTION section, uint32_t idx) {
  switch (section) {
    case CONFIG_COUNT:
      return countConfig.readFloat(idx);
    case CONFIG_CPU:
...
std::string ConfigReader::readString(CONFIG_SECTION section, uint32_t idx) {
  switch (section) {
    case CONFIG_COUNT:
      return countConfig.readString(idx);
    case CONFIG_CPU:
...
bool ConfigReader::readBoolean(CONFIG_SECTION section, uint32_t idx) {
  switch (section) {
    case CONFIG_COUNT:
      return countConfig.readBoolean(idx);
    case CONFIG_CPU:
...
int ConfigReader::parserHandler(void *context, const char *section,
  ...
  bool handled = false;
  if(MATCH_SECTION(SECTION_COUNT)){
    handled = pThis->countConfig.setConfig(name, value);
  }
  ...
  else if (MATCH_SECTION(SECTION_ERROR)) {
    handled = pThis->errorConfig.setConfig(name, value);
  }
