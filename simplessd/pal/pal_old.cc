...
PALOLD::PALOLD(Parameter &p, ConfigReader &c)
  ...
  palasync = new PALasync(stats, &param, &conf, lat);
  palasync->setCommitCallBack([this]() { Commit(); });

  retry.retryStatistics.setRetryModule(retry.config.is_retry);
  retry.retryStatistics.setRequestNumber(0);

  Count::SetOpCount(param.channel, param.package, param.die, param.plane, param.block, param.page);
  Count::Settable(param.channel * param.package * param.die * param.plane * param.block);
...
void PALOLD::read(Request &req, uint64_t &tick) {
  ...
  ::Command cmd(tick, 0, OPER_READ, param.superPageSize, false);
  ...
  outstandingReqQueue.push_back(make_pair(Request(req), list.size()));
  auto Qiter = outstandingReqQueue.end();
  Qiter--;
  ...
  for (auto &iter : list) {
    ...
    palasync->submit(cmd, iter, &Qiter->first, &Qiter->second, &retry);

    stat.readCount++;
    if(req.is_gc) stat.gcReadCount++;

    Count::Addcount(&iter, OPER_READ);
    finishedAt = MAX(finishedAt, cmd.finished);
  }

  tick = finishedAt;
  Qiter->first.finishAt = tick;
  commitCallback();
}

void PALOLD::write(Request &req, uint64_t &tick) {
  uint64_t finishedAt = tick;
  ::Command cmd(tick, 0, OPER_WRITE, param.superPageSize, req.is_multiplane);
  std::vector<::CPDPBP> list;
  ...
  for (auto &iter : list) {
    ...
    palasync->submit(cmd, iter, &Qiter->first, &Qiter->second, &retry);
    stat.writeCount++;
    if(req.is_gc) stat.gcWriteCount++;

    if (cmd.is_multiplane) stat.writeCount += param.plane - 1;
    Count::Addcount(&iter, OPER_WRITE);
    ERROR::update_blockstate(cmd, iter, param);
    finishedAt = MAX(finishedAt, cmd.finished);
  }
...
void PALOLD::erase(Request &req, uint64_t &tick) {
  uint64_t finishedAt = tick;
  ::Command cmd(tick, 0, OPER_ERASE, param.superPageSize * param.page, false);
  std::vector<::CPDPBP> list;
  ...
  for (auto &iter : list) {
    ...
    palasync->submit(cmd, iter, &Qiter->first, &Qiter->second, &retry);
    stat.eraseCount++;
    if(req.is_gc) stat.gcEraseCount++;
  ...
  for (int i = 0; i < 4; i++) {
    ...
    switch (idx) {
      ...
      case INDEX_DIE:
        ...
        break;
      case INDEX_PLANE:
        // AYDNOTICE: now we use a fine-grained multi-plane simulation,
        // this IF is useless.
        if (!useMultiplaneOP || true) { 
...
void PALOLD::getStatList(std::vector<Stats> &list, std::string prefix) {
  ...
  temp.name = prefix + "gc.read.count";
  temp.desc = "Total read operation count caused by GC";
  list.push_back(temp);

  temp.name = prefix + "gc.program.count";
  temp.desc = "Total program operation count caused by GC";
  list.push_back(temp);

  temp.name = prefix + "gc.erase.count";
  temp.desc = "Total erase operation count caused by GC";
  list.push_back(temp);

  temp.name = prefix + "gc.read.bytes";
  temp.desc = "Total read operation bytes caused by GC";
  list.push_back(temp);

  temp.name = prefix + "gc.program.bytes";
  temp.desc = "Total program operation bytes caused by GC";
  list.push_back(temp);

  temp.name = prefix + "gc.erase.bytes";
  temp.desc = "Total erase operation bytes caused by GC";
  list.push_back(temp);
  ...
  palasync->getStatList(list, prefix);
}
...
void PALOLD::getStatValues(std::vector<double> &values) {
  ...
  values.push_back(stat.gcReadCount);
  values.push_back(stat.gcWriteCount);
  values.push_back(stat.gcEraseCount);

  values.push_back(stat.gcReadCount * param.pageSize * planeMultiplier);
  values.push_back(stat.gcWriteCount * param.pageSize * planeMultiplier);
  values.push_back(stat.gcEraseCount * param.pageSize * param.page *
                   planeMultiplier);
...
void PALOLD::write(::CPDPBP &addr, uint64_t &tick) {
  ::Command cmd(tick, 0, OPER_WRITE, param.superPageSize, false);
