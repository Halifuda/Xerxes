...
PAL::PAL(ConfigReader &c) : conf(c) {
  ...
  param.subpageNum = conf.readUint(CONFIG_PAL, NAND_SUBPAGE_NUM);
...
bool PAL::commitToUpper(Request &req) {
  if (commitQueue.empty()) {
    return false;
  }
  auto res = commitQueue.front();
...
void PAL::Commit() {
  ...
  while (pPAL->commitToUpper(req)) {
    ...
    while (iter != submitQueue.end()) {
      if (req.reqID == iter->reqID && req.reqSubID == iter->reqSubID &&
          req.ftlSubID == iter->ftlSubID &&
          req.blockIndex == iter->blockIndex &&
          req.pageIndex == iter->pageIndex) {
        find = true;
        commitQueue.push_back(Request(req));
    ...
    if (!find) {
      panic("Unexpected PAL_OLD commit id=%lu-%lu%lu block%lu-page%lu", 
            req.reqID, req.reqSubID, req.ftlSubID, 
...
void PAL::read(Request &req, uint64_t &tick) {
  submitQueue.push_back(Request(req));
  pPAL->read(req, tick);
}
