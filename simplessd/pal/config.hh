...
typedef enum {
  ...
  /* AYDNOTICE: max suspend times */
  MAX_PAGE_WRITE_SUSPEND,
  MAX_BLOCK_ERASE_SUSPEND,
} PAL_CONFIG;
...
class Config : public BaseConfig {
 public:
  ...
  typedef struct {
    uint64_t loadbuffer;
    uint64_t max_write_suspend;
    uint64_t max_erase_suspend;
  } ISPPConfig;

  typedef struct {
    /// AYDNOTICE: new model
    bool useNewModel;
    ISPPPageTiming new_lsb;
    ISPPPageTiming new_csb;
    ISPPPageTiming new_msb;
    EraseTiming new_erase;
    ISPPConfig isppconf;
    BusTiming cabus;
    BusTiming dbus;
    BusTiming cad_multiplex_bus;
    /// AYDNOTICE: old codes, leave    
    PAGETiming lsb;
    PAGETiming csb;
    PAGETiming msb;
    DMATiming dma0;
 ...
 private:
  ...
  uint32_t subpageNum;          //!< Default: 1
  bool useMultiPlaneOperation;  //!< Default: true
  bool useAMPI;                 //!< Default: false
  bool useSuspend;              //!< Default: false
  bool useSCA;                  //!< Default: false
